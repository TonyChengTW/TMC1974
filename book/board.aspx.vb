       ...
       Dim ilength As Integer = Len(Trim(iOnline))
        ...
        For I = 0 To 3
            ...
            Else
                ...
                CType(Me.FindControl(strImages), Image).ImageUrl = "images/" + Mid(ihits, I + length - 3, 1) + "a.gif"
            End If
        Next
        '今天人數
...
Protected sub CounterDraw()
        ...
        Dim i As Integer
        Dim icounts As Integer
        Dim Cmd As New OleDbCommand("SELECT  COUNT(*)  FROM  talk WHERE (dcreate >= #10/5/2013#)", Conn)
        Conn.Open()
        couters1  = Cmd.ExecuteScalar()
        Label20.text=couters1 
        'Conn.Close()
        Dim Cmd2 As New OleDbCommand("SELECT  Count(*)  FROM talkRe WHERE (talkRe.dcreate)>=#10/5/2013#", Conn)
        'Conn.Open()
