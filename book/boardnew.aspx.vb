...
Partial Class Test
    ...
    Protected Sub CounterDraw()
        ...
        Dim i As Integer
        Dim icounts As Integer
        Dim Cmd As New OleDbCommand("SELECT  COUNT(*)  FROM  talk WHERE (dcreate >= #5/14/2011#)", Conn)
        Conn.Open()
        couters1 = Cmd.ExecuteScalar()
        Label20.text = couters1
        'Conn.Close()
        Dim Cmd2 As New OleDbCommand("SELECT  Count(*)  FROM talkRe WHERE (talkRe.dcreate)>=#5/14/2011#", Conn)
        'Conn.Open()
